...
The algorithm diverges from Octave's `ncauer` in how the design nome `q` is
obtained, then converges back to ncauer's series for everything after that.

ncauer solves the degree equation iteratively via `fminbnd` to get the stopband
edge `ws`, derives the design modulus `k = 1/ws`, and then computes `q = nome(k)`
using the q-series. This implementation instead applies the q-series to the
selectivity modulus `k1 = ep/es` to get `q1 = nome(k1)`, then obtains the design
nome directly via the modular identity `q = q1^(1/N)`. A constexpr iterative
solver would also work -- gcem uses Newton's method internally and is fully
constexpr -- but the modular identity is a direct algebraic consequence of the
nome definition and the degree equation, so no solver is needed at all. The design modulus `k` is
then recovered from `q` via theta functions. From that point on -- the `sig0`
series, the `wi` series, and the pole/zero placement -- the implementation matches
ncauer line for line.
